...
from .models import AllPartiesSelectedColumns
...
def all_parties_with_sale(request, report):
    associated_reports = report.reports.filter(service_name__in=["sale_register", "all_parties"])
    ...
    all_parties_csv = commonutil.get_latest_csv_from_dir(Report.objects.get(service_name="all_parties"))
